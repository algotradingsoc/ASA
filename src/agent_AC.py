...
class RLAgent(Agent):
    ...
    def __init__(self,
        ...
        self.AC = ActorCritic(self.constants, self.session)
    ...
    def on_tick(self, bid, ask):
        ...
        self.AC.memory = self.AC.main_loop(self.AC.memory, inst, lstm, self.orders)
        self.act(self.AC.variables['action'])
        return
    ...
    def on_order(self, order):
        ...
        if self.constants['verbose']:
            print(f"ORDER:\t{self.spread * 1000: .3f}\t{order.type}\t{self.AC.variables['rnd_choice']: }")
        
        inst = self.get_inst_inputs()
        lstm = self.ma_diff_buffer
        self.AC.order_memory = self.AC.main_loop(self.AC.order_memory, 
    ...
    def on_order_close(self, order, profit):
        ...
        if self.constants['verbose']:
            print(f'EXIT: {text},{self.AC.order_epsilon: .5f},{self.AC.empty_epsilon: .5f}')
